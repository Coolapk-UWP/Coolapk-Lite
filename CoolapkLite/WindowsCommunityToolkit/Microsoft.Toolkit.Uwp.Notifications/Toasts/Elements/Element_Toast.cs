...
namespace Microsoft.Toolkit.Uwp.Notifications
{
    internal sealed class Element_Toast : BaseElement, IElement_ToastActivatable, IHaveXmlAdditionalProperties, IHaveXmlName, IHaveXmlNamedProperties, IHaveXmlChildren
    {
        ...
        IEnumerable<KeyValuePair<string, object>> IHaveXmlNamedProperties.EnumerateNamedProperties()
        {
            ...
            {
                yield return new KeyValuePair<string, object>("duration", Duration.ToPascalCaseString());
            }

            yield return new KeyValuePair<string, object>("launch", Launch);

            if (Scenario != DEFAULT_SCENARIO)
            {
                yield return new KeyValuePair<string, object>("scenario", Scenario.ToPascalCaseString());
            }

            yield return new KeyValuePair<string, object>("displayTimestamp", DisplayTimestamp);
            yield return new KeyValuePair<string, object>("hint-toastId", HintToastId);
            yield return new KeyValuePair<string, object>("hint-people", HintPeople);
        }
    }
